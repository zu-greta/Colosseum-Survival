...
@register_agent("student7_agent")
class Student7Agent(Agent):
    """
    ...
    def __init__(self):
        super(Student7Agent, self).__init__()
        self.name = "Student7Agent"
        self.dir_map = {
            "u": 0,
            "r": 1,
            "d": 2,
    ...
    class MCTS():
        ...
        def legal_moves(self, chess_board, my_pos, adv_pos, max_step, root):
            ...
            while state_queue:
                ...
                for dir, move in enumerate(self.moves): #for each direction
                    ...
                    new_node = Student7Agent.MCTS(0, 0, root, (x, y), dir)
                    if over:
                        #print("winner: ", w)
                        if w == 1:
                            #print("win")
                        ...
                        self.remove_barrier(x, y, dir, chess_board) #remove the barrier
                        continue
                    #print("barrier removed: ", chess_board[x, y, dir])
                    self.remove_barrier(x, y, dir, chess_board) #remove the barrier
            ...
            #print("legal_win moves: ", len(legal_win), "legal_very_bad moves: ", len(legal_very_bad), "legal moves: ", len(legal), "legal_bad moves: ", len(legal_bad))
        ...
        def expand(self, my_pos, adv_pos, max_step, chess_board):
            ...
            pos = (x, y)
            node = StudentAgent.MCTS(0, 0, self, pos, dir)
            return node
        """
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        #self.check_valid_step(my_pos, s, d, adv_pos, chess_board, max_step):
            ...
            return my_pos, dir
        """
        # if no good moves, just play a random move (already lost)
        if len(good_children) == 0 and len(bad_children) == 0:
            best_next_move = root.best_move(very_bad) 
        ...
        while((num_childs < len(good_children) or num_childs < len(bad_children)) and not self.timeout(start_time)):
        #for i in range(len(children)):
            #print("DID I NOT GET HERE??")
        ...
        #children = good_children + bad_children
        #print("WHAT?? time: ", time.time() - start_time)
        if len(good_children) == 0:
