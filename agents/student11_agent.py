...
class StudentAgent(Agent):
    ...
    def offensive_moves(self, chess_board, my_pos, adv_pos, max_step):
        ...
        path_queue = [(my_pos, max_step)] # next move options
        visited = [adv_pos] # all nodes visited in correct order
        legal = []
        cur_step = max_step
        while cur_step - 1 and path_queue:
            cur_pos, cur_step = path_queue.pop(0)
            x, y = cur_pos
            ...
            for dir, move in enumerate(self.m):
                ...
                if self.calculate_direction(cur_pos, adv_pos, dir) and sum([chess_board[x, y, i] for i in range(4)]) < 2: 
                    legal.insert(0, [0, 1, cur_pos, dir])
                else:
                ...
                if self.calculate_direction(new_pos, adv_pos, dir) and sum([chess_board[new_x, new_y, i] for i in range(4)]) < 2: 
                    path_queue.insert(0, (new_pos, cur_step - 1))
                else:
                    path_queue.append((new_pos, cur_step - 1)) 
            if (len(legal) > max_step * 4):
                break    
        return legal # NO PRIORITY!
    ...
    def simulate(self, my_pos, dir, adv_pos, max_step, step_board, start_time):
        ...
        while (gameover == False) :
            ...
            if (player_switch == -1) :
                # adversary's turn
                a_children = self.offensive_moves(step_board, adv_pos, my_pos, max_step)
                if len(a_children) == 0:
                ...
                # my turn
                children = self.offensive_moves(step_board, my_pos, adv_pos, max_step)
                if len(children) == 0:
                    gameover, score = self.is_gameover(my_pos, adv_pos, step_board)
                    break
                else:
    ...
    def simulate_old(self, my_pos, dir, adv_pos, max_step, step_board, start_time):
        # take the current step
        self.set_barrier(my_pos[0], my_pos[1], dir, step_board)
        player_switch = -1
        score = 0
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        # children = [] # list of children nodes with each child node = [0: score, 1: num_sims,23: pos, 3: dir]
        # find all legal moves based on my_pos
        #children = self.all_moves(chess_board, my_pos, adv_pos, max_step)
        children = self.offensive_moves(chess_board, my_pos, adv_pos, max_step)
